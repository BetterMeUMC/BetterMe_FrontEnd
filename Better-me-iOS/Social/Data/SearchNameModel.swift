...
class SearchNameModel: ObservableObject{
    ...
    var baseUrl = "http://54.180.13.219:3000/app/follow/searchN/1"
    
    var param: Parameters = ["nickName": "포"]
    ...
    func fetchRandomUsers(){
        print(#fileID, #function, #line, "")
        AF.request(baseUrl, method: .get, parameters: param, headers: token).publishDecodable(type: SearchNameResponse.self)
            .compactMap{ $0.value }
            .map{ $0.result }
