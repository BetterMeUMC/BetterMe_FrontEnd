...
class GoodHabitCell: UICollectionViewCell {
    ...
    @IBAction func tapIsCheckButton(_ sender: UIButton) {
        ...
        if isCheck {
            ...
            self.delegate?.didSelectIsCheck(indexPath: indexPath, completion: completion, isCheck: isCheck)
        } else {
            self.completion -= 1
            self.completionLabel.text = "\(self.completion)/30"
